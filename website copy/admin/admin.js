 ...
 *  Single-file SPA. Reads/writes via MOD_STORE (localStorage-backed).
 * ============================================================================= */
(function () {
  "use strict";

  const CFG = window.MOD_ADMIN_CONFIG || { PASSWORD: "defence2026", SESSION_MINUTES: 60, SESSION_KEY: "mod-admin-session" };
  ...
  function signIn(pw) {
    ...
    localStorage.setItem(CFG.SESSION_KEY, JSON.stringify({ exp: Date.now() + CFG.SESSION_MINUTES * 60 * 1000 }));
    return true;
  }
  function signOut() {
    try { localStorage.removeItem(CFG.SESSION_KEY); } catch {}
    location.reload();
  }
  ...
  function boot() {
    ...
    if (!login || !dash) { console.warn("[admin] login or dashboard not found"); return; }
    ...
    function attemptLogin(e) {
      ...
      const pw = (input.value || "").trim();   // trim invisible whitespace from paste
      console.log("[admin] login attempt, password length =", pw.length);
      if (signIn(pw)) {
        err.textContent = "";
        console.log("[admin] login OK");
        showDashboard();
      } else {
        err.textContent = "Incorrect password. The default is: defence2026";
        err.style.color = "#C8242E";
        err.style.fontWeight = "600";
        console.warn("[admin] wrong password — expected:", (window.MOD_ADMIN_CONFIG && window.MOD_ADMIN_CONFIG.PASSWORD));
        input.select();
      }
      return false;
    }

    // Bind to form submit AND directly to the button click + Enter on input
    // — belt-and-braces so it can't be missed.
    const form = document.getElementById("loginForm");
    const input = document.getElementById("pw");
    const btn = form && form.querySelector("button[type=submit]");
    if (form) form.addEventListener("submit", attemptLogin);
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && !document.getElementById("adminModal").hidden) closeModal();
    });
  }
  ...
  if (document.readyState === "loading") {
    ...
    boot();   // DOM is already ready (script loaded after DOMContentLoaded fired)
  }
  ...
  function renderSlides() {
    ...
    if (!slides.length) { list.innerHTML = `<div class="empty">No slides yet. Click "+ Add slide" above.</div>`; }
    else {
  ...
  function editSlide(idx) {
    ...
    `, function () {
      const updated = { img: getVal("mImg"), role: getVal("mRole"), name: getVal("mName"), alt: getVal("mAlt") };
  ...
  function renderPress() {
    ...
    if (!press.length) { list.innerHTML = `<div class="empty">No press releases yet. Click "+ New press release".</div>`; }
    else {
  ...
  function editPress(idx) {
    ...
    openModal(idx >= 0 ? "Edit press release" : "New press release", `
      <div class="form-grid">
        <div class="form-row"><label>Date</label><input type="text" id="mDate" value="${esc(p.date)}" placeholder="e.g. 15 May 2026" /></div>
        <div class="form-row"><label>Category</label><input type="text" id="mCategory" value="${esc(p.category)}" placeholder="e.g. Press Office" /></div>
        <div class="form-row full"><label>Title</label><input type="text" id="mTitle" value="${esc(p.title)}" /></div>
        <div class="form-row full"><label>Slug (URL anchor)</label><input type="text" id="mSlug" value="${esc(p.slug)}" placeholder="auto-generated if blank" /></div>
        <div class="form-row full"><label>Excerpt</label><textarea id="mExcerpt">${esc(p.excerpt)}</textarea></div>
        <div class="form-row full"><label>Image URL</label><input type="text" id="mImg" value="${esc(p.img)}" /></div>
        <div class="form-row full"><label>External URL (optional)</label><input type="text" id="mUrl" value="${esc(p.url || '')}" placeholder="Link to full article" /></div>
      </div>
    `, function () {
      let slug = getVal("mSlug").trim();
      if (!slug) slug = getVal("mTitle").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60);
      const updated = {
        slug, date: getVal("mDate"), category: getVal("mCategory"),
        title: getVal("mTitle"), excerpt: getVal("mExcerpt"),
        img: getVal("mImg"), url: getVal("mUrl"),
      };
      const all = window.MOD_STORE.press();
      if (idx >= 0) all[idx] = updated; else all.unshift(updated);   // newest first
  ...
  function renderLeadershipEditors() {
    ...
    once("saveMinister", "click", function () {
      window.MOD_STORE.update("leadership.minister", { name: getVal("ministerName"), title: getVal("ministerTitle"), photo: getVal("ministerPhoto") });
      toast("Honourable Minister saved", "success");
    });
    once("saveMos", "click", function () {
      window.MOD_STORE.update("leadership.ministerOfState", { name: getVal("mosName"), title: getVal("mosTitle"), photo: getVal("mosPhoto") });
      toast("Minister of State saved", "success");
    });
    once("savePs", "click", function () {
      window.MOD_STORE.update("leadership.permSec", { name: getVal("psName"), title: getVal("psTitle"), photo: getVal("psPhoto") });
  ...
  function renderSubscribers() {
    ...
    if (!subs.length) { list.innerHTML = `<div class="empty">No subscribers yet. Sign-ups from the public footer form appear here.</div>`; }
    else {
